...
describe('Tweets Slice', () => {
  ...
  it('getTweets should get tweets from server and post actions', async () => {
    const mockedTweets = [
      ...
      {
        "id": 1,
        "tweet": "duis labore tempor eiusmod culpa in ex nostrud magna id qui fugiat ad commodo deserunt occaecat incididunt est proident deserunt",
        "date": "2016-02-20",
        "claps": 30,
        "userId": 1
      }
    ...
    expect(dispatches[1].getAction()).toEqual({ type: 'tweets/tweetsReceived', payload: mockedTweets });
  });
  ...
  it('should handle addingTweetsError action', () => {
    const action = { type: 'tweets/addingTweetsError', payload: new Error('some error') };
    const result = { loading: false, tweets: [], error: new Error('some error') };
    Reducer(TweetReducer).expect(action).toReturnState(result);
  });
